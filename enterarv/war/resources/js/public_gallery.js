...
function gallery(id){
	$("#edit_content_id2").val(id);
	$("#form_move_gallery").submit();
}

function addThumb(content_id, key){
   	$.ajax({
        url: $('#basePath').val()+'image',
        data: { 'blob-key': key },
        dataType: 'json', 
        success: function(data) {
			$("#gallerys").append('<li><a href="'+$("#basePath").val()+'serve?blob-key='+key+'"><img src="'+data.key+'=s250"></a></li>');
...
//        url: 'getGallery.html',
//        data: { 
//        	content_id: $("#edit_content_id").val()
//        },
//        dataType: 'json', 
//        success: function(data) {
//        	if (data.errCode==0){
//        		showGalleryData(data.result);
//        	}
//        },
//        error: function() {
////        	hideLoading();
