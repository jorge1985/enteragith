...
function createMenu(m,c){
    ...
    $.each(m, function(k,v){
        if (v.sub instanceof Array){
            list += '<li><a class="have-menu" data-column="'+c+'" data-index="'+k+'" href="'+v.url+'">'+v.label+' <i class="fa fa-angle-right fa-fw"></i></a></li>';
        }
        else{
...
//        	console.log(data);
	...
	$('#content_table').dataTable( {
		...
		"ajax":	{
			...
			"type": "POST",
			"data" : {
				channel_id : $("#edit_channel_id").val()
			} 
		},
		"order": [[ 1, "desc" ]],
		"columnDefs": [ 
...
//		            { "data": "type_name" },
	...
	$('#content_table').on('click', 'tr', function(){
		var sTable = $("#content_table").dataTable();
		var sData = sTable.fnGetData(this);
		content(sData.id, sData.blog);
	});
});
