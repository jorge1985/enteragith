...
function search(){
	...
	$('#log_table').dataTable().api().ajax.url('loadAccessLog.html?start_day='+start+"&end_day="+end).load();
}
...
function export_csv(){
	...
	$.fileDownload($("#basePath").val()+"export?type=access&start_day=" + start + "&end_day=" + end);
}
...
$(document).ready(function() {
	...
	$('#log_table').dataTable( {
		...
		"responsive": true,
		"ajax":	{
			"url": "loadAccessLog.html?start_day="+getToday()+"&end_day="+getToday(),
			"type": "POST"
		},
		"order": [[ 1, "desc" ]],
        "language": {
