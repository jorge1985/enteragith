...
//			return timestamp.toString();
	...
	/**
	 * Check format of file valid or not.
	 * 
	 * @param type
	 * @param filename
	 * @return boolean
	...
	public static boolean isValidFormat(String type, String filename){
		...
		boolean flag = false;
		for (int i=0; i<Constants.FILE_EXTENTION.length; i++){
			if (filename.endsWith(Constants.FILE_EXTENTION[i])){
				flag = true;
				break;
			}
		}
		...
		for (int i=0; i<Constants.CONTENT_TYPE.length; i++){
			if (type.equals(Constants.CONTENT_TYPE[i])){
				return true;
			}
		}
	...
	public static String getIconOfFileType(String filename){
		filename = filename.toLowerCase();
		...
		if (filename.endsWith(".mp4") || filename.endsWith(".avi") || filename.endsWith(".m4v") || filename.endsWith(".flv") || filename.endsWith(".mov") || filename.endsWith(".f4v"))
		...
		if (filename.endsWith(".tgz") || filename.endsWith(".far"))
	...
	public static boolean canPublish(int userid){
